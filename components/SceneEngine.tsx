...
export const SceneEngine: React.FC<Props> = ({ node, onComplete, onBack, language }) => {
  ...
  const [showDigitalContentModal, setShowDigitalContentModal] = useState(false);
        ...
        return (
          <div className="flex flex-col h-full w-full justify-center items-center p-2">
            <div className={`bg-white/85 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-full overflow-hidden ${introAnim} border border-white/40`}>
              <div className="px-3 py-2 flex justify-between items-center gap-2 flex-shrink-0 bg-white/50 border-b border-gray-100/50">
                <h2 className="text-sm font-black truncate text-blue-900">{node.title}</h2>
              ...
              <div className="p-2 flex justify-center flex-shrink-0 bg-white/50 border-t border-gray-100/50">
          ...
          if (activeSubScene) {
            return (
              <div className="absolute inset-0 z-50 flex flex-col animate-zoom-in bg-slate-900/50 backdrop-blur-sm">
                 ...
                 <div className="relative z-10 flex flex-col p-2 w-full max-w-4xl mx-auto h-full justify-end">
                     ...
                     <div className="flex justify-center mt-2 flex-shrink-0 pt-1 pb-2">
                        <Button onClick={() => { playSfx('click'); setVisitedSubScenes(prev => new Set(prev).add(activeSubScene.id)); setActiveSubScene(null); }} className="py-1.5 px-8 rounded-full text-xs font-bold shadow-xl bg-blue-600 text-white border-2 border-white/20">
          ...
          return (
            <div className={`h-full flex flex-col items-center max-w-5xl mx-auto w-full relative z-10 ${animClass}`}>
               <div className="flex-shrink-0 mb-2">
                 <h3 className="text-xs font-black text-white text-center drop-shadow-lg bg-black/40 px-3 py-1 rounded-xl backdrop-blur-md border border-white/10 w-fit mx-auto">
                    ...
                    {data.subScenes.map(scene => {
                      ...
                      return (
                        <button key={scene.id} onClick={() => { playSfx('modal_open'); setActiveSubScene(scene); }}
                          ...
                          `}
                        >
                           <div className="h-[70%] w-full relative overflow-hidden bg-slate-800 flex-shrink-0">
                              ...
                              <div className="absolute top-1 right-1 text-xl drop-shadow-md">{scene.icon}</div>
                              {isVisited && (
                                 <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-[1px]">
                                    <div className="bg-green-500 text-white rounded-full p-1 shadow-lg border-2 border-white text-xs">✓</div>
                                 </div>
                              )}
                           </div>
                           <div className={`h-[30%] w-full flex flex-col items-center justify-center p-1 text-center transition-colors flex-grow ${isVisited ? 'bg-slate-100' : 'bg-white/95 backdrop-blur-md'}`}>
                              <span className="font-black text-[10px] text-blue-900 leading-none mb-0.5 truncate w-full px-1">{scene.title}</span>
                           </div>
                        </button>
                      );
                    })}
        ...
        if (data.interactionType === InteractionType.MULTIPLE_CHOICE && data.interactionData?.question) {
           return (
             <div className={`h-full w-full flex flex-col items-center overflow-hidden relative ${animClass}`}>
                <div className="flex-shrink-0 mb-2 z-10 max-w-[98%] mt-1">
                   <div className="bg-white/85 backdrop-blur-xl text-blue-900 rounded-lg shadow-lg p-2 text-center border border-white/40 w-fit mx-auto">
                      <h3 className="text-xs font-black relative z-10 leading-tight">{data.interactionData.question}</h3>
                   </div>
                </div>
                
                <div className="flex-1 min-h-0 w-full max-w-xl flex flex-col items-center relative overflow-hidden">
                  {!interactionComplete ? (
                        ...
                        {data.interactionData.answers.map((ans: any) => {
                          ...
                          return (
                            <button key={ans.id} onClick={() => handleMultipleChoice(ans)}
                              className={`py-2 px-3 rounded-xl font-bold text-xs sm:text-sm transition-all flex justify-between items-center text-start shadow-sm shrink-0 min-h-[40px] leading-tight backdrop-blur-sm w-full
                            ...
                            >
                              ...
                              <span className="text-sm flex-shrink-0 ml-2">{isSelected && ans.correct && '✓'}{isError && '✕'}</span>
                            </button>
                          );
                        })}
                     </div>
        ...
        if (data.interactionType === InteractionType.DRAG_SHIELD) {
            return (
                <div className="h-full w-full flex flex-col items-center relative animate-zoom-in">
                    <div className="flex-shrink-0 mb-1 mt-1">
                        {!interactionComplete && <h3 className="text-center bg-black/40 text-white backdrop-blur-md rounded-full py-1 px-4 self-center text-[10px] font-bold border border-white/10 w-fit">{t('shieldInst', language)}</h3>}
                    </div>
                    
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 flex-1 overflow-y-auto min-h-0 p-1 w-full max-w-4xl items-center content-center">
                        {interactionItems.map((item) => (
                            <button 
                                key={item.id} 
                                onClick={() => handleShieldClick(item)}
                                disabled={item.protected}
                                className={`relative rounded-xl flex flex-col items-center text-center shadow-lg transition-all w-full overflow-hidden group max-w-[150px] mx-auto h-full max-h-[140px]
                                ...
                                `}
                            >
                                <div className="w-full flex-1 bg-white relative p-1 min-h-[50px]">
                                    {item.image ? (
                                        <img src={item.image} className="w-full h-full object-contain" alt="" />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center text-gray-400">?</div>
                                    )}
                                    {item.protected && <div className="absolute inset-0 flex items-center justify-center bg-green-500/30 z-20"><span className="text-2xl animate-bounce">🛡️</span></div>}
                                </div>
                                <div className="p-1 w-full flex-shrink-0 flex items-center justify-center min-h-[25px] bg-slate-50 border-t border-slate-100">
                                     <span className="text-[9px] font-bold text-gray-900 leading-tight">{item.text}</span>
                                </div>
                            </button>
                        ))}
                    </div>
                    ...
                    {interactionComplete && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-md z-20 animate-pop-in">
                            <div className="bg-white/95 p-3 rounded-2xl text-center shadow-2xl w-fit max-w-xs border border-white/50">
                                ...
                                <h3 className="font-black text-blue-900 text-xs mb-2">{t('correct', language)}</h3>
                                {isTrivialDecision && trivialOption ? (
                                    <div className="bg-blue-50 p-2 rounded-lg mb-2 text-[10px] text-gray-800 text-right leading-normal border border-blue-100">
        ...
        if (data.interactionType === InteractionType.BALLOONS) {
            return (
                <div className="h-full w-full flex flex-col relative overflow-hidden rounded-xl animate-zoom-in">
                    ...
                    <div className="flex-shrink-0 mt-1 mb-1 text-center">
                        {!interactionComplete && <h3 className="inline-block bg-black/40 text-white backdrop-blur-md rounded-full py-1 px-4 text-[10px] font-bold z-10 shadow-md border border-white/10">{t('balloonsInst', language)}</h3>}
                    </div>
                        ...
                        {interactionItems.map((item, index) => {
                             ...
                             return (
                                 <button
                                    ...
                                    className={`absolute w-20 h-24 sm:w-28 sm:h-32 flex flex-col items-center justify-center text-center p-1 cursor-pointer transition-transform active:scale-95 balloon-float
                                 ...
                                 >
                                     <div className={`absolute inset-0 balloon-shape ${index % 2 === 0 ? 'bg-red-500' : index % 3 === 0 ? 'bg-blue-500' : 'bg-yellow-400'} overflow-hidden`}></div>
                                     <span className="relative z-10 text-[9px] font-bold text-white leading-tight drop-shadow-md px-1 bg-black/20 rounded backdrop-blur-[1px]">{item.text}</span>
                                     <div className="absolute top-full left-1/2 w-[1px] h-10 bg-white/50 origin-top animate-[stringWave_3s_ease-in-out_infinite]"></div>
                                 </button>
                             );
                        })}
                    </div>
        ...
        if (data.interactionType === InteractionType.CODE_CRACKER) {
                    ...
                    <div className="h-full w-full flex flex-col items-center justify-center relative animate-zoom-in overflow-hidden">
                        <div className="bg-white/95 p-4 rounded-2xl shadow-2xl text-center backdrop-blur-xl border border-white/50 w-fit max-w-2xl flex flex-col items-center z-10 max-h-full my-auto">
                             ...
                             <div className="overflow-y-auto custom-scrollbar flex-1 min-h-0 mb-4 px-2 w-full">
                                <p className="text-xs sm:text-sm leading-relaxed text-gray-800 font-medium whitespace-pre-wrap text-center">{data.moreInfoContent}</p>
                             </div>
            ...
            return (
                <div className={`h-full w-full flex flex-col relative ${animClass} items-center`}>
                    <div className="w-fit flex justify-between items-center gap-3 bg-black/40 px-3 py-1 rounded-xl backdrop-blur-md mb-2 mt-1 border border-white/10 shrink-0">
                        <span className="text-white font-mono font-bold text-xs">{t('codeLabel', language)}:</span>
                        <div className="flex gap-1.5">
                            {[0, 1, 2, 3].map(i => (
                                <div key={i} className={`w-6 h-8 bg-slate-800 rounded flex items-center justify-center text-white font-mono text-base border-2 ${i < collectedCode.length ? 'border-green-400 text-green-400' : 'border-slate-600'}`}>
                    ...
                    {currentQ && (
                        <div className="flex-1 w-full max-w-xl bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl flex flex-col overflow-hidden relative border border-white/40 mb-2 mx-2 min-h-0">
                             ...
                             <div className="p-2 text-center border-b bg-white/50 shrink-0">
                                 <h3 className="font-bold text-blue-900 text-xs sm:text-sm leading-tight">{currentQ.question}</h3>
                             </div>
                             ...
                             {showCodeFeedback ? (
                                 <div className="flex-1 flex flex-col animate-fade-in overflow-hidden relative min-h-0">
                                     <div className="flex-1 overflow-y-auto px-4 py-2 flex flex-col items-center justify-center text-center custom-scrollbar">
                                         <div className="text-xl mb-1 flex-shrink-0">💡</div>
                                         <p className="text-xs font-bold mb-2 leading-relaxed text-gray-800">{codeFeedbackText}</p>
                                         <div className="text-2xl font-mono font-black text-blue-600 tracking-widest bg-blue-50 px-4 py-1 rounded-lg border border-blue-200 flex-shrink-0">
                                             {pendingDigit}
                                         </div>
                                     </div>
                                     <div className="p-2 border-t bg-gray-50 flex-shrink-0 z-10 w-full">
                                         <Button onClick={confirmCodeDigit} fullWidth className="animate-bounce shadow-xl py-1.5 text-xs">{t('next', language)}</Button>
                                     </div>
                                 </div>
                             ) : (
                                 <div className="flex-1 grid grid-cols-1 gap-1.5 p-2 overflow-y-auto custom-scrollbar content-center">
                                     {currentQ.options.map((opt: any, idx: number) => (
                                         <button 
                                            key={idx}
                                            onClick={() => handleCodeAnswerSelection(opt.value, currentQ.explanation)}
                                            className={`p-2 rounded-xl font-bold text-xs shadow-sm transition-all border-2 border-transparent flex items-center justify-center text-center h-full min-h-[36px]
      ...
      case 'DECISION_PHASE_2':
        ...
        return (
          <div className={`h-full w-full flex flex-col items-center overflow-hidden ${animClass} relative justify-center max-w-4xl mx-auto`}>
             {!selectedOption && (
                 <div className={`bg-white/85 backdrop-blur-xl text-blue-900 rounded-lg shadow-lg relative overflow-hidden transition-all flex-shrink-0 flex flex-col justify-center text-center border border-white/40 w-fit max-w-[95%] mx-auto p-2 mb-1 mt-1`}>
                    <p className={`relative z-10 leading-tight font-bold text-xs sm:text-sm line-clamp-3`}>{questionText}</p>
                 </div>
             )}

             <div className="flex-1 min-h-0 flex flex-col items-center relative w-full overflow-hidden justify-center">
               {!selectedOption ? (
                  <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2 p-1 pb-1 w-full max-w-lg justify-center">
                    {optionsList?.map(opt => (
                       <button key={opt.id} disabled={!!selectedOption}
                         className="py-2 px-3 text-start rounded-xl transition-all shadow-md font-bold text-gray-800 text-xs bg-white/90 hover:bg-blue-50 hover:scale-[1.01] min-h-[36px] flex items-center group flex-shrink-0 leading-tight backdrop-blur-sm w-full border border-white/20"
                  ...
                  <div className="flex flex-col animate-pop-in overflow-hidden w-full max-w-md mx-auto p-2 h-full justify-center">
                     <div className="bg-blue-50/90 border-r-4 border-blue-500 p-2 rounded-lg mb-2 flex-shrink-0 shadow-sm backdrop-blur-sm w-full">
                         <span className="text-[9px] font-bold text-blue-400 uppercase tracking-wider block mb-0.5">{t('yourChoice', language)}</span>
                     ...
                     <div className="bg-white/90 shadow-xl rounded-xl flex flex-col items-center text-center relative overflow-hidden border border-white/50 backdrop-blur-xl w-full flex-1 min-h-0">
                        <div className="p-3 flex flex-col items-center overflow-y-auto custom-scrollbar flex-1 min-h-0">
                            ...
                            <p className="text-xs text-gray-800 font-medium leading-relaxed px-1 whitespace-pre-wrap">{feedbackText}</p>
                        </div>
                        <div className="w-full flex flex-col gap-1.5 p-2 bg-slate-50/80 border-t border-gray-100 flex-shrink-0 z-10">
                           <Button variant="outline" className="w-full py-1 text-[10px] text-blue-700 font-bold border-0 bg-blue-50/50" onClick={() => { playSfx('modal_open'); setShowMoreInfo(true); }}>ℹ️ {t('moreInfo', language)}</Button>
                           <Button className="w-full py-1.5 text-xs shadow-lg bg-blue-600 text-white rounded-lg font-black" onClick={handleDecisionComplete}>
             ...
             {showMoreInfo && (
               <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-md animate-fade-in">
                 <div className="bg-white/95 w-full max-w-lg max-h-[90vh] flex flex-col rounded-2xl p-4 shadow-2xl relative border border-white/50 animate-zoom-in">
                   <button onClick={() => { playSfx('modal_open'); setShowMoreInfo(false); }} className="absolute top-2 left-2 w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center font-bold text-xs z-10 hover:bg-gray-200 transition-colors">✕</button>
                   <h3 className="text-sm font-black text-blue-900 mb-2 border-b pb-2 pl-8 leading-tight text-right">{moreInfoTitle}</h3>
                   <div className="overflow-y-auto flex-1 mb-2 pl-1 custom-scrollbar">
                      <p className="text-gray-900 text-xs leading-relaxed whitespace-pre-line font-medium text-right">{moreInfoContent}</p>
                   </div>
                   <Button fullWidth onClick={() => { playSfx('modal_open'); setShowMoreInfo(false); }} className="py-2 text-xs font-black shadow-lg rounded-xl flex-shrink-0">{t('understood', language)}</Button>
                 </div>
               </div>
             )}
          </div>
  ...
  return (
    <div className="h-full w-full flex flex-col font-sans relative overflow-hidden bg-slate-900" onClick={handleBackgroundClick}>
       ...
       <div className={`p-1.5 flex justify-between items-center z-20 transition-all flex-shrink-0 h-10 bg-transparent pointer-events-none ${isIntro && !showIntroCard ? 'opacity-0' : 'opacity-100'}`}>
         <div className="flex gap-2 pointer-events-auto">
             <Button variant="outline" className={`py-0.5 px-2 text-[10px] rounded-full backdrop-blur-md font-bold shadow-lg bg-white/20 border-0 text-white hover:bg-white/30`} onClick={onBack}>
                {t('backToMap', language)}
             </Button>
             {!isIntro && (
                <Button 
                    variant="outline" 
                    className="py-0.5 px-2 text-[10px] rounded-full backdrop-blur-md font-bold shadow-lg bg-white/20 border-0 text-white hover:bg-white/30 flex items-center gap-1"
             ...
             {node.data.digitalContent && !isIntro && (
                <Button 
                    variant="secondary" 
                    className="py-0.5 px-2 text-[10px] rounded-full shadow-lg bg-yellow-400 text-blue-900 font-bold animate-pulse"
         ...
         {!isIntro && <span className="font-bold text-white text-[10px] drop-shadow-md bg-black/40 px-2 py-0.5 rounded-full truncate max-w-[120px] backdrop-blur-md pointer-events-auto border border-white/20">{node.title}</span>}
       </div>
